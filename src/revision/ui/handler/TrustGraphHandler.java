...
public class TrustGraphHandler {
	 ...
	 */
	public static ArrayList<String> addReportAll(ArrayList<JTextField> t_formula, ArrayList<JTextField> t_result, DistanceState distance) {
		...
		for (int i = 0; i < t_formula.size(); i++)
		{
			...
			{
				try {
					newmsg = addReport(t_formula.get(i).getText(), Integer.parseInt(t_result.get(i).getText()), distance);
	 ...
	 * 
	 */
	private static ArrayList<String> addReport(String form, int res, DistanceState distance) throws ParserException {
		Report r;
